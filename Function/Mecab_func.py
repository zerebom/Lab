...
def get_word(colum,path,sheet):
    ...
    for i in range(1,200):
        ...
        # print(i)
        # print(sheet.cell(row=i,column=colum).value)
        # print(word_dict)
...
def count_people(speechList,char_num):
    for speech in speechList:
        for (key ,value) in char_num.items():
            ...
            if str(key) in str(speech):
                char_num[key]+=1
                # print('yeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee')
    print(char_num)
    ...
    #     # 辞書作成、リストに追加
    #     morpheme =
    #         'surface': cols[0],#←ハイパーパラメータ
    #         'base': res_cols[6],#←ハイパーパラメータ
    #         'pos': res_cols[0],#←ハイパーパラメータ
    #         'pos1': res_cols[1]#←ハイパーパラメータ
    #     }
    #     morphemes.append(res_cols[6])
    #
    #     # 品詞細分類1が'句点'なら文の終わりと判定
    #     if res_cols[1] == '句点':
    #         speech_list2.append(morphemes)
    #         morphemes = []
    # return speech_list2
